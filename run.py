...
def register_hotkeys():
    """Register or re-register all hotkeys safely."""
    try:
        if hasattr(keyboard, "clear_all_hotkeys"):
            keyboard.clear_all_hotkeys()
        else:
            # Fall back to older APIs for compatibility
            keyboard.unhook_all()
            if hasattr(keyboard, "remove_all_hotkeys"):
                keyboard.remove_all_hotkeys()
    except Exception:
        # Some versions might not implement clearing; ignore failures
        pass
